...
function Element:New(Config)
    ...
    local function ClickAnim(down)
        if not CanCallback then return end
        Creator.Tween(Button.ButtonFrame.UIElements.Container, .1, {
            Size = down and UDim2.new(1, -6, 1, -2) or UDim2.new(1, 0, 1, 0)
        }):Play()
    end

    Creator.AddSignal(Button.ButtonFrame.UIElements.Main.MouseButton1Down, function()
        ClickAnim(true)
    end)
    Creator.AddSignal(Button.ButtonFrame.UIElements.Main.MouseButton1Up, function()
        ClickAnim(false)
    end)
    Creator.AddSignal(Button.ButtonFrame.UIElements.Main.MouseLeave, function()
        ClickAnim(false)
    end)
