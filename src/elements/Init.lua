return {
    ...
    Load = function(tbl, Container, Elements, Window, HyperUI, OnElementCreateFunction, ElementsModule, UIScale, Tab)
        for name, module in next, Elements do
            tbl[name] = function(self, config)
                            ...
                            if ConfigManager.Parser[data.__type] then
                                task.defer(function()
                                    ...
                                    if success then
                                        ...
                                        warn("[ HyperUI ] Failed to apply pending config for '" .. config.Flag .. "': " .. tostring(err))
                                    end
                                end)
                            end
                        end
                ...
                if frame and frame.UIElements then
