...
function Creator.Init(HyperUITable)
...
function Creator.SafeCallback(Function, ...)
    ...
    if not Success then
        if HyperUI and HyperUI.Window and HyperUI.Window.Debug then
            local _, i = Event:find(":%d+: ")
        
            warn("[ HyperUI: DEBUG Mode ] " .. Event)
            
            return HyperUI:Notify({
...
function Creator.Gradient(stops, props)
    if HyperUI and HyperUI.Gradient then
        return HyperUI:Gradient(stops, props)
    end
...
function Creator.New(Name, Properties, Children)
    ...
    for _, Child in next, Children or {} do
        -- Prevent "Parent property is locked" errors
        if typeof(Child) == "Instance" and not (Child.Parent and Child.Parent ~= Object) then
...
function Creator.Image(Img, Name, Corner, Folder, Type, IsThemeTag, Themed, ThemeTagName)
    ...
    elseif string.find(Img,"http") then
        local FileName = "HyperUI/" .. Folder .. "/assets/." .. Type .. "-" .. Name .. ".png"
        task.spawn(function()
            ...
            if needsDownload then
                ...
                if success and response and response.Body and response.Body ~= "" then
                    if not RunService:IsStudio() and writefile then 
                        pcall(writefile, FileName, response.Body) 
                    end
                else
                    warn("[ HyperUI.Creator ] Failed to download asset: " .. tostring(Img))
                    return
                end
            end
            
            if ImageFrame and ImageFrame:FindFirstChild("ImageLabel") then
                ...
                if assetSuccess then
                    ...
                    warn(string.format("[ HyperUI.Creator ] Failed to load custom asset '%s': %s", FileName, tostring(asset)))
                end
            end
        end)
    elseif Img == "" then
