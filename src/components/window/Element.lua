...
return function(Config)
    ...
    if Element.Image then
        ...
        if typeof(Element.Color) == "string" then 
            ImageFrame.ImageLabel.ImageColor3 = Creator.GetTextColorForHSB(Color3.fromHex(Creator.Colors[Element.Color]))
        elseif typeof(Element.Color) == "Color3" then
            ImageFrame.ImageLabel.ImageColor3 = Creator.GetTextColorForHSB(Element.Color)
        end
    ...
    local function CreateText(Title, Type)
        local TextColor = typeof(Element.Color) == "string" 
            and Creator.GetTextColorForHSB(Color3.fromHex(Creator.Colors[Element.Color]))
            or typeof(Element.Color) == "Color3" 
            and Creator.GetTextColorForHSB(Element.Color)
    ...
    local ElementFullFrame = New("Frame", {
        ...
        ZIndex = Creator.ZIndex.Elements,
    })
    ...
    if Element.Hover then
        Creator.AddSignal(Main.MouseEnter, function()
            if CanHover then
                Tween(Main, .15, {ImageTransparency = Element.Color and .15 or .9}):Play()
                Tween(Hover, .15, {ImageTransparency = .92}):Play()
                Tween(HoverOutline, .15, {ImageTransparency = .85}):Play()
                Creator.AddSignal(Main.MouseMoved, function(x,y)
                    local relativeX = ((x - Main.AbsolutePosition.X) / Main.AbsoluteSize.X) - 0.5
                    Hover.HoverGradient.Offset = Vector2.new(relativeX, 0)
                    HoverOutline.HoverGradient.Offset = Vector2.new(relativeX, 0)
                end)
            end
        end)
        Creator.AddSignal(Main.InputEnded, function()
            if CanHover then
                Tween(Main, .15, {ImageTransparency = Element.Color and .05 or .93}):Play()
                Tween(Hover, .15, {ImageTransparency = 1}):Play()
                Tween(HoverOutline, .15, {ImageTransparency = 1}):Play()
            end
        end)
    end
