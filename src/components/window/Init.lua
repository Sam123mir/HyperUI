...
return function(Config)
    ...
    local Window = {
        ...
        TopBarButtonIconSize = Config.TopBarButtonIconSize or 22,
    ...
    Window.TopBarButtonIconSize = Window.TopBarButtonIconSize or (Window.Topbar.ButtonsType == "Mac" and 9 or 22)
    ...
    if not RunService:IsStudio() and Window.Folder and writefile then
        Window.Folder = Creator.SanitizeFilename(Window.Folder)
        if not isfolder("HyperUI/" .. Window.Folder) then
            makefolder("HyperUI/" .. Window.Folder)
        end
        if not isfolder("HyperUI/" .. Window.Folder .. "/assets") then
            makefolder("HyperUI/" .. Window.Folder .. "/assets")
        end
    end
    ...
    local UserIcon = Sidebar.Init(Window, Config)
    ...
    if typeof(Window.Background) == "string" and BGVideo then
        ...
        if string.find(BGVideo, "http") then
            ...
            if not isfile(videoPath) then
                ...
                if not success then
                    warn("[ HyperUI.Window.Background ] Failed to download video: " .. tostring(result))
                    return
                end
            end
            ...
            if not success then
                warn("[ HyperUI.Window.Background ] Failed to load custom asset: " .. tostring(customAsset))
                return
            end
    ...
    local WindowTitleContainer = New("Frame", {
        BackgroundTransparency = 1,
        AutomaticSize = "XY",
        Name = "TitleRow",
        LayoutOrder = 1,
    ...
    -- / Tags / Badges /
    if Config.Tags and typeof(Config.Tags) == "table" then
        task.spawn(function()
            task.wait(0.1) -- wait for topbar to render
            for _, tag in ipairs(Config.Tags) do
                if tag.Title then
                    local tagColor = tag.Color or Color3.fromRGB(100, 200, 255)
                    New("Frame", {
                        AutomaticSize = "XY",
    ...
    Window.UIElements.Main = New("Frame", {
        ...
        HyperUI and HyperUI.UIScaleObj or New("UIScale", { Scale = 1 }),
    ...
    if not IsVideoBG and Window.Background and typeof(Window.Background) == "table" then
        ...
        Window.UIElements.BackgroundGradient = Creator.NewRoundFrame(Window.UICorner, "Squircle", {
            ...
            ImageTransparency = Window.Transparent and Config.HyperUI.TransparencyValue or 0
        }, {
            BackgroundGradient
        })
    end
    ...
    function Window:SetBackgroundTransparency(v)
        ...
        Config.HyperUI.TransparencyValue = rounded
    ...
    if Config.HyperUI.UseAcrylic then
    ...
    function Window:Open()
        task.spawn(function()
            ...
            if Window._FooterOpenCallback then
                task.spawn(function()
                    Window._FooterOpenCallback()
                end)
            end
            ...
            task.spawn(function()
                ...
                Config.HyperUI:ToggleAcrylic(true)
            end)
        end)
    end
    function Window:Close()
        ...
        if Window._FooterCloseCallback then
            task.spawn(function()
                Window._FooterCloseCallback()
            end)
        end
        
        Config.HyperUI:ToggleAcrylic(false)
        ...
        local CurrentPosition = Window.UIElements.Main.Position
        ...
        task.spawn(function()
            ...
            if Window.OpenButtonMain and not Window.Destroyed and Window.IsOpenButtonEnabled then
                Window.OpenButtonMain:Visible(true, CurrentPosition)
            end
        end)
        ...
        function Close:Destroy()
            task.spawn(function()
                ...
                if Window._FooterDestroyCallback then
                    task.spawn(function()
                        Window._FooterDestroyCallback()
                    end)
                end
    ...
    function Window:GetUIScale(v)
        return Config.HyperUI.UIScale 
    end
    
    function Window:SetUIScale(v)
        Config.HyperUI.UIScale = v
        Tween(Config.HyperUI.UIScaleObj, .2, {Scale = v}, Enum.EasingStyle.Quint, Enum.EasingDirection.Out):Play()
        return Window
    end
    ...
    Creator.AddSignal(UserInputService.InputBegan, function(input, isProcessed)
        ...
        if ControlDown and input.KeyCode == Enum.KeyCode.H then
            Window:Toggle()
        end
    end)
    ...
    local TabModule = TabModuleMain.Init(Window, Config.HyperUI, Config.HyperUI.TooltipGui)
    ...
    function Window:Tab(TabConfig)
        ...
        return TabModule.New(TabConfig, Config.HyperUI.UIScale)
    end
    ...
    function Window:Section(SectionConfig)
        return SectionModule.New(SectionConfig, Window.UIElements.SideBar.Frame, Window.Folder, Config.HyperUI.UIScale, Window)
    end
    ...
    function Window:Dialog(DialogConfig)
        ...
        local function CheckButtonsOverflow()
            ...
            local totalWidth = ButtonsLayout.AbsoluteContentSize.X / Config.HyperUI.UIScale
            local parentWidth = ButtonsContent.AbsoluteSize.X / Config.HyperUI.UIScale
            ...
            if totalWidth > parentWidth then
                ...
                if availableSpace > 0 then
                    ...
                    for _, button in ipairs(Buttons) do
                        local buttonWidth = button.AbsoluteSize.X / Config.HyperUI.UIScale
    ...
    Resize.Init(Window, ResizeHandle, FullScreenIcon)
    ...
    -- / Footer /
    if Config.StatsWidget ~= false then
        local Footer = require("./modules/Footer")
        Footer.Init(Window)
    end
