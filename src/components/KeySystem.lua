...
function KeySystem.new(Config, Filename, func, keyValidator)
    ...
    -- (Hidden Key System text removed for cleaner layout)
    ...
    if Config.KeySystem.URL then
        CreateButton("Get Link Key", "key", function()
            setclipboard(Config.KeySystem.URL)
        end, "Secondary", ButtonsContainer.Frame)
    end
    
    if Config.KeySystem.Discord or Config.Discord then
        local discordLink = Config.KeySystem.Discord or Config.Discord
        CreateButton("Join Discord", "tv", function()
    ...
    if Config.KeySystem.API then
        ...
        for _, i in next, Config.KeySystem.API do
            local serviceDef = Config.HyperUI.Services[i.Type]
            if serviceDef then
                local args = {}
    ...
    local SubmitButton = CreateButton("Validate Key", "check-circle", function()
        ...
        elseif not Config.KeySystem.API then            
            ...
            local isKey = type(keyToCheck) == "table"
                and table.find(keyToCheck, key)
                or tostring(keyToCheck) == key
